...
- Treat `artifact.record(..., include_recent_inbound_messages=True)` as the queued human-message mailbox: when it returns user input, prioritize that input over the current background subtask until it has been acknowledged and incorporated.
- If the user request is directly answerable, answer it in that immediate follow-up and prefer `artifact.record(payload={'kind': 'answer'}, ...)` over hiding the answer inside a generic `progress` update.
...
- Write the real user-facing `artifact.record(...)` message in full. Do not manually turn the actual message into a preview by inserting `...` / `…`, dropping the conclusion tail, or stripping away the key comparison; the runtime can derive a shorter preview separately.
- During active foreground work, send `artifact.record(payload={'kind': 'progress'|'milestone', 'reply_mode': 'threaded'}, ...)` at real checkpoints and usually within about `10-20` meaningful tool calls once user-visible state changed; after a state-changing artifact tool or a clear subtask boundary, send one immediately.
...
`artifact.record(...)` is the main human-feedback MCP and the main long-lived user-visible thread across web, TUI, and bound connectors.
...
- treat `artifact.record(..., include_recent_inbound_messages=True)` as the queued human-message mailbox
- if it returns `recent_inbound_messages`, those messages become the highest-priority user instruction bundle
- immediately send one substantive follow-up `artifact.record(...)`
...
- soft trigger: after about `10` meaningful tool calls, if there is already a human-meaningful delta, send `artifact.record(payload={'kind': 'progress'}, reply_mode='threaded', ...)`
...
- true quest completion still requires an explicit completion-approval flow followed by `a Hermes-controlled finalize recommendation record(...)`
...
- `artifact.record(...)` for user-visible continuity; use `kind='answer'` for direct questions, `kind='progress'` for checkpoints, `kind='milestone'` for material state changes, and `kind='decision_request'` only for real blockers
...
- `a Hermes-controlled branch activation action(...)` for branch/worktree routing
...
- `a Hermes-controlled finalize recommendation record(...)` only after explicit user approval
...
- `artifact.record(..., include_recent_inbound_messages=True)` is the mailbox poll; after any non-empty poll, immediately send one substantive follow-up and do not send a receipt-only filler line.
...
- The default long-run monitoring cadence is about `60s -> 120s -> 300s -> 600s -> 1800s -> 1800s ...`; after each sleep/await cycle, inspect `bash_exec(mode='list')` and `bash_exec(mode='read', id=...)`, compare against the previous evidence, then decide whether a fresh `artifact.record(...)` is actually needed.
...
2. if the extra evidence should attach to an older durable branch, first call `a Hermes-controlled branch activation action(...)` for that branch
...
- the current workspace can intentionally differ from the latest research head after `a Hermes-controlled branch activation action(...)`
...
- Finalize is not quest completion by default. `a Hermes-controlled finalize recommendation record(...)` is allowed only after explicit user approval.
...
6. Supplementary evidence
   ...
   - First MCP pattern:
     - `artifact.resolve_runtime_refs(...)`
     - if needed `a Hermes-controlled branch activation action(...)`
...
9. Closure
   ...
   - First MCP pattern:
     ...
     - `Hermes/plugin graph inspection(...)`
   - Must classify supported / partial / unsupported / deferred outcomes explicitly.
   - Must not call `a Hermes-controlled finalize recommendation record(...)` without explicit completion approval.
...
- Never call `a Hermes-controlled finalize recommendation record(...)` just because one turn, one stage, one run, or one checkpoint finished.
