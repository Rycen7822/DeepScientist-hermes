...
This is a class-level companion for DeepScientist experiment execution. If the plugin-provided `deepscientist:experiment` skill is available, load it first and treat this skill as local operational guidance for pitfalls and reusable checks discovered in sessions.

## Core rules

- Use Hermes/DeepScientist native `ds_*` tools for durable quest state and bash execution; do not call an external `ds` CLI.
...
- When ongoing experiment documentation becomes too long for reliable continuation, use `deepscientist:quest-handoffs` / `deepscientist-quest-handoffs`'s `references/concise-current-status-maintenance.md` pattern: create a short current-status entry, point long command/plan docs to it, preserve source-of-truth history, and verify stale handoff text is gone.
