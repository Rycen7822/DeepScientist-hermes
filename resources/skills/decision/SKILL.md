...
- If `startup_contract.decision_policy = autonomous`, do not emit ordinary `artifact.record(payload={'kind': 'decision_request'}, ...)` calls; decide the route yourself, record the reason, and continue.
...
- Quest completion is a special terminal decision: first ask for explicit completion approval with `artifact.record(payload={'kind': 'decision_request'}, reply_mode='blocking', reply_schema={'decision_type': 'quest_completion_approval'}, ...)`, and only after an explicit approval reply should you call `a Hermes-controlled finalize recommendation record(...)`.
...
- **For git state inside the current quest repository or worktree, prefer `quest-local git state via backend service or Hermes-controlled git action(...)` before raw shell git commands.**
- **Use `decision` to judge the route, not as an excuse to bypass the `bash_exec(...)` / `quest-local git state via backend service or Hermes-controlled git action(...)` tool contract.**
...
- return to an older durable branch without creating a new node -> `a Hermes-controlled branch activation action(...)`
...
When a decision materially changes the route, follow it with the appropriate user-visible `artifact.record(...)` update:

- use threaded `artifact.record(payload={'kind': 'milestone'}, reply_mode='threaded', ...)` when the decision is already durably resolved and the quest can continue automatically
...
If user input is needed, also use `artifact.record(payload={'kind': 'decision_request'}, ...)`.
