...
> DeepScientist plugin note: this is a plugin-owned support skill, adapted from the user-local Hermes skill library. Use native `ds_*` tools for durable quest state, memory, artifacts, and quest-local shell execution. Load it only when it is the relevant companion to the active stage.
...
- Replace legacy shell-wrapper calls with Hermes `ds_bash_exec` for quest-logged shell; ordinary file tools for direct file IO.
...
- Replace legacy memory helper calls with `session_search(...)`, Hermes `memory(...)`, and durable local review files as appropriate.
- Route names such as `intake-audit`, `scout`, `analysis-campaign`, `baseline`, `write`, `decision`, `finalize`, and `rebuttal` are workflow labels. If those exact skills are unavailable, use the closest available Hermes skills/tools to accomplish the same purpose.
...
- Hard execution rule: if this stage needs shell work such as document builds, scripted checks, Git inspection, or file inspection, every such command must go through the active execution surface. In ordinary Hermes use `ds_bash_exec` for quest-logged shell; ordinary file tools for direct file IO; in DeepScientist mode use `ds_bash_exec` so quest state, logs, and artifacts stay durable.
...
- write the follow-up work into `paper/review/experiment_todo.md` and mirror it into Hermes `todo` when active execution is needed
...
Open additional skills only when the review workflow requires them. These route names are conceptual; if the exact skill is unavailable in Hermes, use the closest available skill or direct tool workflow.
...
- if the review produced a durable lesson, claim downgrade, revision rule, or experiment-gap judgment that will matter across sessions, write at least one concise Hermes `memory(...)` entry
