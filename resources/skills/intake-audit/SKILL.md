...
- When the audit reaches a durable route recommendation, send one richer `artifact.record(payload={'kind': 'milestone'}, reply_mode='threaded', ...)` update that says what state is trusted, what still needs work, and which anchor should run next.
...
- **For git inspection or maintenance inside the current quest repository or worktree, prefer `quest-local git state via backend service or Hermes-controlled git action(...)` before raw shell git commands.**
...
At the end of the intake pass, send one threaded `artifact.record(payload={'kind': 'milestone'}, ...)` update that says:
