...
- When scouting actually resolves the framing ambiguity, locks the evaluation contract, or makes the next anchor obvious, send one richer `artifact.record(payload={'kind': 'milestone'}, reply_mode='threaded', ...)` update that says what is now clear, why it matters, and which stage should come next.
...
- **For git inspection inside the current quest repository or worktree, prefer `quest-local git state via backend service or Hermes-controlled git action(...)` before raw shell git commands.**
...
Use `artifact.record(...)` for a structured decision request only when a real ambiguity remains and local evidence cannot safely resolve it.
