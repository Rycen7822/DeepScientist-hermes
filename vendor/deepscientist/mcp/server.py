...
ARTIFACT_STATE_CHANGE_WATCHDOG_NOTES = {
    "confirm_baseline": (
        ...
        "summary on its own. Write one concise artifact.record(...) progress record now so Hermes can surface it."
    ),
    "waive_baseline": (
        "Baseline waiver changed durable quest state and this tool does not send a user-visible summary "
        "on its own. Write one concise artifact.record(...) progress record now so Hermes can surface it."
    ),
    "submit_paper_outline": (
        "Paper outline state changed durably and this tool does not send a user-visible summary on its own. "
        "Write one concise artifact.record(...) progress record now so Hermes can surface it."
    ),
    "attach_baseline": (
...
def _progress_watchdog_note(tool_call_count: int) -> str:
    return (
        ...
        f"{tool_call_count} tool calls without writing a user-visible artifact.record(...) progress record. "
...
def _visibility_watchdog_note(seconds_since_last_update: int) -> str:
    ...
    return (
        ...
        f"{minutes} minutes since the last user-visible artifact.record(...) progress record. "
...
def build_memory_server(context: McpContext) -> FastMCP:
    ...
    server = FastMCP(
        ...
        instructions=(
            ...
            "do not promote quest-local memory to global state from MCP, "
            "and prefer quest-local scope when quest context exists."
        ),
        log_level="ERROR",
    )
